...
const userCart = asyncHandler(async (req, res) => {
    const { cart } = req.body;
    const { _id } = req.user;
    // const _id = cart[0]._id;
    console.log("sdsdsd", _id);
    validateMongodbId(_id);
    try {
        let products = []
        const user = await User.findById(_id);
        ...
        console.log("cart exists", alreadyExistCart);
        if (alreadyExistCart) {
...
const applyCoupon = asyncHandler(async (req, res) => {
    ...
    await cartModel.findOneAndUpdate({ order: user._id }, { totalAfterDiscount }, { new: true });
